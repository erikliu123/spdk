                                        ...
                                        void *cb_arg)
{
    ...
    if (ndp_req->read_bdev_blocks == ndp_req->total_bdev_blocks)
    {
        ...
        SPDK_INFOLOG(ndp, "IO consume: %ld ticks, per seconed ticks: %ld, HEADER[%x]\n", ndp_req->end_io_time - ndp_req->start_io_time, ticks_per_second, ndp_req->ptr.read_ptr[0]
        );
#if 0        
        ...
        SPDK_INFOLOG(ndp, "###accel:%d###\tNumber of faces detected: %u\n", ndp_req->accel, dets.size());

        //获取人脸特征点分布
        if(face_feature_flag){//之前是因为ndp_request malloc的大小过小带来内存泄漏
            int i = 0;
        ...
        SPDK_INFOLOG(ndp,"#####face detection call finished!#####\n");
        spdk_nvmf_request_complete(req);
